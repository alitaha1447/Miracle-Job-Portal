...
const navItems: NavItem[] = [
  {
    ...
    subItems: [
      ...
      { name: "Job Dashboard", path: "/", },
      // { name: "Paricipant Dashboard", path: "/participants-list", },
...
const AppSidebar: React.FC = () => {
  ...
  // Exact keys match what you store in localStorage: "Student" | "Company" | "College"
  const ALLOW: Record<string, string[]> = {
    Student: ["Student Dashboard"],
    College: ["College Dashboard"],
    Company: ["Job Dashboard"], // your rule
  };
    ...
    return (
      <ul className="flex flex-col gap-4">
        {items.map((nav, index) => {
          ...
          const subItems =
            isDashboard && roleType && ALLOW[roleType]
              ? (nav.subItems || []).filter((s) => ALLOW[roleType].includes(s.name))
              : nav.subItems;

          if (isDashboard && roleType && ALLOW[roleType] && (!subItems || subItems.length === 0)) {
            return null;
          }
          // ✅ For Dashboard: show subItems directly (no dropdown/toggle)
          if (isDashboard && subItems) {
            return (
              <li key={nav.name}>
                  ...
                  <ul className="mt-2 space-y-1 ml-0">
                    {subItems.map((subItem) => (
                      <li key={subItem.name}>
                        ...
                        >
                          <span className={`menu-item-icon-size ${isActive(subItem.path)
                            ? "menu-item-icon-active"
                            : "menu-item-icon-inactive"
                            }`}>
                            {nav.icon /* ✅ show icon here */}
                          </span>
                          {/* {(isExpanded || isHovered || isMobileOpen) && ( */}
          ...
          // 🔽 Default behavior for items that DO have subItems (keep your dropdown)
          if (subItems) {
            return (
              <li key={nav.name}>
                <button
          ...
          // 🔗 Items without subItems (simple link)
          return (
            nav.path && (
              <li key={nav.name}>
                <Link
